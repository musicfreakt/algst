...
//        tableSelect = new JComboBox(new String[]{"Рабочие", "Менеджеры", "Клиенты", "Контракты", "Профессии"});
...
//        if (scroll != null)
//            app.remove(scroll); // для случая инициализации
//        String[] columns = null;
//        String[][] data = null;
//        switch (select) {
//            case ("Рабочие"):
...
//                break;
//            case ("Клиенты"):
//                columns = new String[] {"ID", "Имя", "Фамилия", "Компания"};
//                List<Client> clientsList = em.createQuery("SELECT c FROM clients c").getResultList();
//                data = new String[clientsList.size()][3];
...
//                break;
//            case ("Профессии"):
//                columns = new String[] {"ID", "Название"};
//                List<Specialisation> specList = em.createQuery("SELECT s FROM specialisations s").getResultList();
//                data = new String[specList.size()][2];
//                for (int i = 0; i < specList.size(); i++)
//                {
//                    data[i] = specList.get(i).toTableFormat();
//                }
//                break;
//        }
...
//
//                editPanel.setLayout(new GridLayout(1, 2, 5, 1));
...
////        return true;
////    }
////    private boolean addEmployee(String name, String surname, int exp) {
////        if (!em.getTransaction().isActive())
////            em.getTransaction().begin();
////        Employee model = new Employee(10, name, surname, exp);
////        em.persist(model);
////        em.getTransaction().commit();
////        DefaultTableModel dtm = (DefaultTableModel) table.getModel();
////        dtm.addRow(model.toTableFormat());
...
////        return true;
////    }
//    private boolean addSpecialisation(String name)
//    {
//        if (!em.getTransaction().isActive())
//            em.getTransaction().begin();
//        Specialisation model = new Specialisation(name);
//        em.persist(model);
//        em.getTransaction().commit();
//        DefaultTableModel dtm = (DefaultTableModel) table.getModel();
//        dtm.addRow(model.toTableFormat());
...
//                        addFlag = addSpecialisation(name);
//                        break;
//                }
//                if (addFlag == true) {
//                    // закрыть окно JDialog и вывести сообщение об успешном добавлении
